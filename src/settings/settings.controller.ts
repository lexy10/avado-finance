...
export class SettingsController {
  ...
  async getVerificationStatus(@Req() request: Request, @Res() response: Response) {
    try {
      const verification_status = await this.settingsService.getVerificationStatus(request.body)
      response.status(HttpStatus.OK).json({
        status: true,
  ...
  @Post('/submit-verification')
    async submitVerification(@Req() request: Request, @Res() response: Response) {
      try {
        ...
        if (!isBase64(requestBody.verification_id_image) || !isBase64(requestBody.verification_liveliness_image))
          throw new BadRequestException('Image must be in base64 format')

        const submittedVerification = await this.settingsService.getVerificationStatus(requestBody)
        response.status(HttpStatus.OK).json({
          status: true,
